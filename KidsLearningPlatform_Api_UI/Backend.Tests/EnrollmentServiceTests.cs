...
namespace Backend.Tests
{
  public class EnrollmentServiceTests
  {
    ...
    public async Task Enroll_ReturnsDto_WhenNewEnrollment()
    {
      ...
      _repo.Setup(r => r.GetByKidId(1))
           .ReturnsAsync(new List<Enrollment>()); // no duplicates

      _repo.Setup(r => r.Add(It.IsAny<Enrollment>()))
    ...
    {
      // Arrange
      var existing = new Enrollment { EnrollmentId = 10, KidId = 1, CourseId = 2 };
    ...
    public async Task GetEnrollmentsByKid_MapsKidAndCourse_ToDto()
    {
      ...
      // IMPORTANT: your repository already Includes Kid & Course in real code.
      // For the unit test we just return objects with those nav props filled.
