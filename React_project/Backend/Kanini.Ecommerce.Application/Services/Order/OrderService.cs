...
public class OrderService : IOrderService
{
    ...
    private List<object> GetTrackingSteps(string status)
    {
        var steps = new List<object>
        {
            new { Step = "Order Placed", Status = "Completed", Date = DateTime.Now.AddDays(-3) },
            new { Step = "Order Confirmed", Status = status == "Pending" ? "Pending" : "Completed", Date = status == "Pending" ? (DateTime?)null : DateTime.Now.AddDays(-2) },
            new { Step = "Processing", Status = GetStepStatus(status, "Processing"), Date = GetStepStatus(status, "Processing") == "Completed" ? DateTime.Now.AddDays(-1) : (DateTime?)null },
            new { Step = "Shipped", Status = GetStepStatus(status, "Shipped"), Date = GetStepStatus(status, "Shipped") == "Completed" ? DateTime.Now : (DateTime?)null },
            new { Step = "Delivered", Status = GetStepStatus(status, "Delivered"), Date = GetStepStatus(status, "Delivered") == "Completed" ? DateTime.Now.AddDays(1) : (DateTime?)null }
        };
        return steps;
    }

    private string GetStepStatus(string currentStatus, string stepStatus)
    {
        var statusOrder = new[] { "Pending", "Confirmed", "Processing", "Shipped", "Delivered" };
        var currentIndex = Array.IndexOf(statusOrder, currentStatus);
        var stepIndex = Array.IndexOf(statusOrder, stepStatus);
        
        if (currentIndex >= stepIndex) return "Completed";
        if (currentIndex == stepIndex - 1) return "In Progress";
        return "Pending";
    }
