...
public class JwtOtpService : IJwtOtpService
{
    ...
    public string CreateOtpTokenWithData(string email, string otp, string otpType, RegistrationDataDto registrationData)
    {
        try
        {
            var secretKey = _configuration[MagicStrings.ConfigKeys.JwtSecretKey];
    ...
    public RegistrationDataDto? GetRegistrationDataFromToken(string jwtToken)
    {
        try
        {
            ...
            var jwtSecurityToken = (JwtSecurityToken)validatedToken;
            var registrationDataJson = jwtSecurityToken.Claims.FirstOrDefault(x => x.Type == "registrationData")?.Value;
            ...
            var registrationData = JsonSerializer.Deserialize<RegistrationDataDto>(registrationDataJson);
